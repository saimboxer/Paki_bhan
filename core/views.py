...
class stopdetail(APIView):
    def get(self, request, stopid, **kwargs):
        ...
        serializer = StopDetailSerializer(
            stoplist, many=False, context={"pstopid": stopid}
        )
        resp = {"status": "success", "data": serializer.data}
...
class TripSchedule(APIView):
    def post(self, request, pfrom, pto, **kwargs):
        buseslist = RouteDetail.objects.raw(
            "select distinct route_id, id from core_RouteDetail where location_id in (5,7);"
        )
